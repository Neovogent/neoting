...
DECLARE
  ...
  tenant_tables text[] := ARRAY[
    ...
    'document_links', 'chat_conversations', 'teams', 'team_members'
  ];
BEGIN
...
-- teams / team_members: the firm's own org chart (review item 54, 7 Sep 2026).
--
-- `teams` has a practice and NO business — a team is not about a client — so it
-- is not on the direct_tables loop, and its predicate is the anchor-pair
-- function called with a NULL business. That is preferred over the
-- invites/guidance two-branch shape written inline for the otp_sessions reason:
-- app_can_access_document() carries the `app_session_scope() = 'user' AND
-- app_actor_id() IS NOT NULL` guards, so a delegated portal session can never
-- read a practice's org chart. The NULL is cast because an untyped NULL cannot
-- resolve the overload.
--
-- ⚠ **Neither table is ever consulted BY a policy.** A team grants no access;
-- membership of one changes nothing about what its members can reach. Who may
-- reach which client is `memberships`, and that is the only answer any
-- predicate in this file asks for. If a future policy ever joins `team_members`
-- to decide visibility, that is the moment this file acquired a second tenancy
-- model, and it should not survive review.
DROP POLICY IF EXISTS teams_tenant ON teams;
CREATE POLICY teams_tenant ON teams
  USING (app_can_access_document(NULL::text, practice_id))
  WITH CHECK (app_can_access_document(NULL::text, practice_id));

-- The child, reached through its parent — the chase_messages shape. A row whose
-- team is out of reach is out of reach, so a team id guessed from elsewhere
-- buys nothing.
DROP POLICY IF EXISTS team_members_tenant ON team_members;
CREATE POLICY team_members_tenant ON team_members
  USING (EXISTS (SELECT 1 FROM teams t WHERE t.id = team_id AND app_can_access_document(NULL::text, t.practice_id)))
  WITH CHECK (EXISTS (SELECT 1 FROM teams t WHERE t.id = team_id AND app_can_access_document(NULL::text, t.practice_id)));
