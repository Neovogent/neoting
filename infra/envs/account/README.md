...
| 2 | **`neoting-pot-8000` starts 2025-08-01.** Runbook Step 10.1 specifies Aug 2026. | The cumulative window opens twelve months before Neoting existed. With gap 1, this is why the pot reads ~$897 while the monthly budget reads ~$115. One-line fix, but it changes a number the CEO reads, so it gets its own diff. | **High** |
