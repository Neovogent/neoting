...
- **`envs/prod/` is written but unapplied — applying it is Slice C (Infra Week, G8).** The three blockers this bullet used to list have all landed since: the alarm estate (`observability.tf`, 15 Aug — described above), the `nt_app` non-owning role (`prisma/sql/app-role.sql` + `envs/prod/db-app-role.tf`, exercised by CI's tenancy stage), and the state-bucket `role/nt-*` deny (`envs/account/tfstate.tf`, 15 Aug). What still gates the first apply is scheduling and money (runbook Appendix B budgets prod from ~Oct), not missing config.
- ~~**The prod GitHub environment does not exist yet.**~~ — created 17 Aug 2026 with a deployment branch policy restricting it to `main`, so `nt-prod-ci-deploy` is now assumable. ⚠ **Required reviewers could NOT be set**: the API returns 422 "billing plan" — the same free-plan limitation that blocks branch protection. Until the org moves to Team, the `prod` environment gates on branch only, not on a human click; §14.9's one-click promote joins the branch-protection deferral list. Re-run the reviewer PUT the day the plan changes.
...
- ~~**⚠ `nt-staging-ci-deploy` can read and overwrite `prod/core.tfstate` and `account/core.tfstate`.**~~ — narrowed to `staging/` on 16 Aug 2026, **before** the first prod apply rather than after it. Prod's state holds `random_password` values in plaintext (the Redis auth token, the `nt_app` role password), so once prod existed this would have been a live path from the environment that runs on every push to main straight to production credentials. Closing it once it is real is incident response; closing it while it is theoretical is a one-word permission change. The plan showed exactly one statement moving, `.../\*` → `.../staging/\*`.
  ...
  ~~⚠ **Residual, and smaller but real:** `nt-staging-ci-plan` can still **read** `prod/core.tfstate`~~ — closed 17 Aug 2026, and not the way this note predicted: `ReadOnlyAccess` stays (enumerating every read a refresh needs is a policy that breaks with the next module), and an **explicit Deny** on `prod/*` and `account/*` in the state bucket now overrides it (`deny-foreign-state-reads`, main.tf). The role reads exactly one state prefix: its own.
...
- **The remaining alerting gap is subscribers, not topics.** `nt-staging-alerts`, `nt-prod-page` and `nt-prod-ticket` all exist and all have correct publish policies. Email subscriptions for all three were created 17 Aug 2026 (shakibbinkabir@gmail.com) and sit in `PendingConfirmation` until the confirmation links in that inbox are clicked — an unconfirmed subscription delivers nothing. Verify with `aws sns list-subscriptions-by-topic --topic-arn <arn>`; treat any `PendingConfirmation` as unsubscribed. One human inbox is still a single point of failure — runbook §10.1 wants a role address alongside it.
...
- ~~**SES production access**~~ — **granted.** Verified 17 Aug 2026: `sesv2 get-account` reports `ProductionAccessEnabled: true`, `SendingEnabled: true`, enforcement `HEALTHY` (case `178662887400793`). Outbound email is unblocked; the go-live gate for actually sending remains D33 (budget line + usage metric + alert per vendor).
