...
**✅ RESOLVED — deterministic layer (5 Sep 2026, correction-integrity package; the MODEL-backed second opinion stays with items 19/48).** Mubashir's ruling implemented as written — warn, offer Ignore, never hard-block:
...
Verified end to end in the app: retyping the £9,000 tax warns, Ignore stages, the review restates, and correcting it back makes the July 2025 export succeed (see item 29 below).
...
**✅ (a), (b) and (d) RESOLVED (5 Sep 2026, correction-integrity package).** (a) is item 22's warning layer (see its entry). (b): the release review now leads with a **"⚠ Checks — read before you release"** section — a document that will produce NO line in the export file is the FIRST thing on the card and is counted in the card's title ("⚠ 1 document will produce no export line"), and the sign-mismatch refusal message is augmented at proposal time with the plain sentence ("Tax £9000.00 is larger than the total £994.00 — correct the tax or the total, then propose the release again"). The advisory is applied identically at creation and at the executor's recompute (`applyEntryAdvisories` in `publish-batch.ts` — `sameEntryPreview` fingerprints it, so one-sided application would refuse every approval). (d) verified live: with the tax corrected back, the 30/07/2025 export succeeds. (c) — the export refusal naming the document and the fix path — stays with package C (item 29's copy half). The exporter itself is unchanged, as ruled.
...
**✅ The FLAG half is RESOLVED (5 Sep 2026, correction-integrity package):** a corrected document date in the future — or more than 7 years past — now warns at the correction boundary with the item-22 [Ignore — I'm sure] escape, client-side in the dialog and restated on the proposal review; the "100%" display is "Confirmed by you" now (item 22's entry). The date-PICKER half (UK d/m/y control, better UX) stays with package D (items 16/28/46's shared control) — not this package's.
...
**✅ RESOLVED — 1, 2 and 3; 4's model pass stays with items 19/48 (5 Sep 2026, correction-integrity package).**
