...
**Point 4 is NOT built, and it is named rather than quietly dropped.** `buildSupplierRuleProposal` — the "make it a rule?" payload — has been built and tested since A6 and **has no caller in the product**, because this module publishes no controller. Offering it needs a contract operation (G7). Supplier memory now delivers the behaviour the feature was wanted for; formalising a repeated treatment into a standing rule is the part still owed, recorded in `rules-suggestions/CLAUDE.md`'s TODO.
