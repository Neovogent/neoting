...
```bash
export AWS_PROFILE=nt
aws sesv2 get-account --region eu-west-2 --query 'ProductionAccessEnabled'
aws service-quotas list-requested-service-quota-change-history \
  --service-code textract --region eu-west-2 \
  --query 'RequestedQuotas[].{Quota:QuotaName,Status:Status,Desired:DesiredValue}' --output table
```

`CASE_OPENED` means still pending. `APPROVED`/`CASE_CLOSED` at 10.0 means granted — and that is when **verification 8.4 closes and ADR 0003 becomes writable** (runbook §12.2 lists it as blocked on exactly this).
