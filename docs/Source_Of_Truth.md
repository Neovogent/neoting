...
| D22 | Model access route: **Claude via Amazon Bedrock, eu-west-2** (single cloud, IAM auth, residency). W0 verification: Opus 4.8 + effort parameter availability in eu-west-2 Bedrock; contingency = Anthropic API with EU processing terms, decision logged. **STT = Amazon Transcribe streaming (eu-west-2, en-GB).** |
...
| AI services | **Amazon Textract (eu-west-2)** — `AnalyzeExpense` + `AnalyzeDocument` — behind `DocumentExtractor`, with the **vision escalation ladder** (Textract → Sonnet-vision → Opus-vision → human) for below-threshold documents (D20/D28). **Three Claude tiers via Amazon Bedrock (eu-west-2):** Haiku 4.5 mechanical · Sonnet 4.6 volume workhorse · Opus 4.8 judgment — temperature 0, JSON-schema-enforced, **effort set per task class** (D28/D22). **Amazon Transcribe streaming** for voice (D22). | One cloud, one region, IAM auth; pinned model + prompt versions recorded per extraction; UK/EU processing; no-training DPAs; **no first-party training/fine-tuning (D19)**; blended pipeline cost guardrail **< £0.02 per document**; chat spend under per-firm budgets. |
...
**Tenancy model.** Every row carries its workspace identity; Postgres RLS enforces the **practice → client** hierarchy below the application layer: practice staff read across their practice's clients only per assignment policy; client users see their own workspace; **delegated OTP sessions are scoped to exactly the requested items** and nothing else. S3 keys are workspace-prefixed with per-workspace KMS encryption context. Cross-tenant access is structurally impossible and continuously tested for (governance §15).
...
**Cost guardrail (restored, D28):** blended pipeline AI cost **< £0.02 per document** at v1 volumes — indicative composition at current list pricing: Textract `AnalyzeExpense` ~£0.008/page · Haiku triage ~£0.002 · Sonnet medium-effort coding-suggestion call with prompt caching ~£0.006–0.010 · amortised Opus usage (final vision rung + occasional rule parsing) ~£0.002–0.005 — verified against actual Bedrock pricing at W0 and reviewed with pilot data. **Chat-workspace spend sits under the per-firm daily budgets, not this per-document figure.** Pricing note (CEO): the documents/month fair-use ceiling remains a pricing input, now with comfortable headroom.
...
| **Amazon Bedrock (Claude Opus 4.8 · Sonnet 4.6 · Haiku 4.5)** | All LLM tasks, three tiers (D28/D22) | eu-west-2; IAM auth; model-access enablement is a W0 request; **W0 verification: all three models + per-model effort support in-region** — contingency is the Anthropic API with EU processing terms, decision logged. |
...
| **AWS Bedrock model access** (Claude Opus 4.8, Haiku 4.5) | Enablement request: hours–days | **W0 verification item (D22):** in-region availability + effort parameter; contingency route logged if not. |
