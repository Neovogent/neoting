...
**Version 1.7 · 15 September 2026 · Confidential**
...
*Changelog v1.6 → v1.7: **Initial Delivery has shipped.** This file described ID as the release being built, in the present tense, which meant every reader — human or agent — arrived believing the ledger lane was forbidden. **D50 supersedes D42**: the second release adds a live ledger connection for **Xero, QuickBooks Online, Sage Business Cloud Accounting and FreeAgent**, and export stops being the *sole* egress while remaining a permanent one. ⚠ **What this entry deliberately does not claim:** the second release has no recorded name, date or wider scope, and **whether D40 (manual bank upload only) and D48 (subscription) still hold is not known to this author** — they are left standing until someone who knows says otherwise, rather than quietly amended. ⚠ **Correction carried forward:** the v1.5 → v1.6 entry above records D48's price as **€8.50**; every other artefact — this file's §24, `CLAUDE.md`, the published privacy notice and the legal pack — says **£8.50**. The pound is correct; the euro is a typo in the changelog prose only, left in place above because rewriting history is worse than annotating it.*
...
| **D50** | **The second release connects to the ledger — supersedes D42.** ID has shipped. The second release adds a live **API connection for Xero, QuickBooks Online, Sage Business Cloud Accounting and FreeAgent**: a practice connects a client business once, and an approved transaction is created in that client's books **with the source document attached**, returning the vendor's own reference as proof. **Export is no longer the sole egress — but it remains a permanent one**, because VT Transaction+ has no API and never will, and neither does anything else in Tier C of `docs/research/ledger-input-api-request-list.md`. This is a **second** egress, not a replacement. **D43 is unchanged and gets easier**: through a file the source link is a typable capability URL; through an API it is a real file attachment on the transaction, which is the rung D43's ladder always aimed at. **Unchanged:** nothing may write to a ledger outside the Review → Approve path, and Approve stays server-enforced. **Standing groundwork:** developer accounts, registered apps, redirect URIs and sandboxes exist for all four (`docs/research/ledger-input-api-request-list.md` §14); credentials are in `.env.integrations`; the API detail is in `docs/research/ledger-api-build-reference.md`; the build brief is `docs/ledger-connect-build-brief.md`. ⚠ **Not yet built:** the OAuth flow, token vault, refresh scheduler, `ReferenceSync` service and connection UI do not exist in the repo — only the `LedgerAdapter` seam and a demo implementation behind it. |
...
> ⚠ **Superseded by D50 (v1.7) — ID has shipped and the second release connects to the ledger.** The text below described ID: export only, no adapter, no API, no auto-publish. *Published* becomes an internal state meaning approved and released for export, and every exported line must carry a resolvable link to its source document.
...
> ⚠ **ID has shipped (v1.7, 15 Sep 2026).** This section is now the **record of a delivered release**, not a description of work in progress. Read it in the past tense. **D50 supersedes D42** — the second release connects to the ledger; see `docs/ledger-connect-build-brief.md`.
