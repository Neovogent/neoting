...
| `document.update-coding` — **all fields** | 2 | **2** | ⚠ **Returned to tier 2 on 9 Sep 2026**, on the owner's third statement of the same rule: *"No approval will be required for anything, except for when the document is going for publishing. I have corrected this twice already. Now I'm saying it for the third time."* Offered the literal reading or keeping the record without the wait, he chose literal. Item 22's £9,000-of-tax case is answered by the review the server still renders, records and hashes — what moved is whose signature it carries. Gate ⚖5 (which fields) is now moot and closed. |
