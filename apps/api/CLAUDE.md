...
**Staging runs the real ingest lane since 17 Aug 2026:** the task definitions set `INGEST_QUEUE=bullmq`, `OBJECT_STORE=s3`, `IMAGE_NORMALISER=sharp` and `AUTH_MODE=session` (infra/envs/staging/services.tf). Still fixture there, by design: `EMAIL_SOURCE` (the s3 poller in `worker/email-intake-main.ts` has no ECS service yet) and `DOCUMENT_GUARD` (qpdf is not in the image). METH Stage 15 (#146) added the rest of the demo environment: `SESSION_SECRET`, `PORTAL_LINK_SECRET` and `PORTAL_SESSION_SECRET` injected from the `/neoting/staging/auth` secret, and `EXTRACTOR`/`SMS_SENDER`/`OTP_MODE`/`LEDGER_ADAPTER` stated as `demo` rather than left to their defaults. Until that landed, every session request on staging failed closed — `env.ts` deliberately refuses to make those secrets a boot requirement, so a missing one cannot crash-loop a deploy's `/healthz`; it still refuses the header-trusting fixture resolver under `NODE_ENV=production`.
