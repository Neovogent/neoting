...
**`EXTRACTOR` and `AI_CHAT` have a third value, `replay`:** the REAL Bedrock adapters run with `messages.create` served from `apps/api/fixtures/cassettes/bedrock/` (`common/bedrock-replay.ts`) — same Zod parse, §9.2 retry, error classification and budget metering, offline. A cassette miss fails loudly naming `pnpm --filter @neoting/api record:cassettes` (`--live` records the real model, pennies); it never falls through to live Bedrock. Refused under `NODE_ENV=production`, tests pinning both refusals. Cassettes store the response and the request's hash only — no request bodies, so no document bytes or prompts in fixtures — and every write is structurally redacted. Editing a prompt, tool schema or the model pin orphans every cassette key: the replay tests then fail demanding a re-record, which is the mechanism working (the eval-recording property, one seam lower).

**⚠ Most of the committed set is LIVE as of 6 Sep 2026, and the one that is not is the one that never could be.** The coding-intelligence work (review items 19/48) added a third corpus — `rules-suggestions/coding/coding-replay-corpus.ts`, the coding rung and the correction second opinion — and **neither of those sends bytes**, so a `--live` re-record of them is not blocked. Running it also re-recorded the extraction OCR-text path and the chat happy turn live. What remains synthetic, and why:

| Cassette | Why |
|---|---|
| extraction `receipt-image` | ⚠ **the standing blocker.** `replay-corpus.ts`'s byte-path item sends `ONE_PIXEL_PNG`, which the live API refuses with a 400 (measured 5 Sep 2026, re-confirmed 6 Sep). Swapping in a small real receipt image is still the prerequisite. `--live` does NOT crash on it — the extractor classifies the 400 as `NT-EXT-009`, writes no cassette, and the run continues |
| extraction `malformed-answer`, chat `schema-retry`, coding `hostile-answer` | always synthetic by design — a real model cannot be made to misbehave on demand, and these exist to drive the real refusal paths (a bad Zod parse, §9.2's retry, an off-chart code with a borrowed basis and a markup-shaped sentence) |

Stale cassettes from earlier prompt versions are left in place: the recorder never deletes, and an orphan fails nothing.
