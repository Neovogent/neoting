...
Consequently staging sets `EXTRACTOR=bedrock`, `OTP_MODE=totp` and `DOCUMENT_GUARD=qpdf` (with the binary now in the image), and two of those cost something until the stages behind them land: **PDFs fail with NT-EXT-003 until A4** teaches BedrockExtractor the document content block, and **nobody can sign in until A2** implements the real verifier, because both TOTP checks read `mode === 'demo'` and therefore return false. Both are fail-closed and both are the intended intermediate state. Still fixture on staging, by design: `EMAIL_SOURCE` (the s3 poller in `worker/email-intake-main.ts` has no ECS service yet).
