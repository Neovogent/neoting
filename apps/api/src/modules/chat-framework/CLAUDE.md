...
**Lane J** · **Source of Truth:** SoT §8, §16 · **Owner:** see the project board

## Purpose

The chat workspace, the grammar-emission service, and the Review-Approve card mechanics.
...
- The model emits specs against `packages/component-grammar` — never free-form UI. A spec that fails validation is not rendered, and the failure is logged with its trace.
...
```bash
pnpm --filter @neoting/api test -- chat-framework
```
