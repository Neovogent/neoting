...
const READY_BATCH: readonly PublishPreviewItem[] = [
  { docType: 'INVOICE' as const, id: 'doc_001', supplierName: 'Bidfood', categoryCode: 'Cost of Sales — Food', totalPence: 128_450, taxPence: 21_408, currency: 'GBP' },
  { docType: 'INVOICE' as const, id: 'doc_007', supplierName: 'British Gas', categoryCode: 'Utilities', totalPence: 41_266, taxPence: 6_878, currency: 'GBP' },
  { docType: 'INVOICE' as const, id: 'doc_012', supplierName: 'Bidfood', categoryCode: 'Cost of Sales — Food', totalPence: 8_840, taxPence: 1_473, currency: 'GBP' },
];
...
test('a null VAT contributes nothing and does not block the batch', () => {
  const preview = computePublishPreview([
    { docType: 'INVOICE' as const, id: 'd', supplierName: 'Nisbets', categoryCode: 'Equipment', totalPence: 5_000, taxPence: null, currency: 'GBP' },
  ]);
...
test('one shared currency survives onto the preview', () => {
  const preview = computePublishPreview([
    { docType: 'INVOICE' as const, id: 'a', supplierName: 'Nexora', categoryCode: 'Subscriptions', totalPence: 5_435_251, taxPence: 0, currency: 'usd' },
    { docType: 'INVOICE' as const, id: 'b', supplierName: 'Nexora', categoryCode: 'Subscriptions', totalPence: 100, taxPence: 0, currency: 'USD' },
  ]);
...
test('a batch that mixes currencies has NO currency, and that is not sterling', () => {
  const preview = computePublishPreview([
    { docType: 'INVOICE' as const, id: 'a', supplierName: 'Nexora', categoryCode: 'Subscriptions', totalPence: 5_435_251, taxPence: 0, currency: 'USD' },
    { docType: 'INVOICE' as const, id: 'b', supplierName: 'Bidfood', categoryCode: 'Cost of Sales — Food', totalPence: 128_450, taxPence: 21_408, currency: 'GBP' },
  ]);
...
test('a batch recording no currency at all is null, never assumed sterling', () => {
  expect(computePublishPreview([{ docType: 'INVOICE' as const, id: 'a', supplierName: 'Bidfood', categoryCode: 'Food', totalPence: 100, taxPence: 0 }]).currency).toBeNull();
  expect(computePublishPreview([{ docType: 'INVOICE' as const, id: 'a', supplierName: 'Bidfood', categoryCode: 'Food', totalPence: 100, taxPence: 0, currency: '' }]).currency).toBeNull();
});
...
test('a £0.00 total refuses like a missing one — the minimum follows readiness (2026-09-03)', () => {
  ...
  const refusal = checkPublishMinimum({ docType: 'INVOICE' as const, id: 'd', supplierName: 'Bidfood', categoryCode: 'Cost of Sales — Food', totalPence: 0, taxPence: 0 });
  expect(refusal?.missing).toEqual(['total']);
  expect(checkPublishMinimum({ docType: 'INVOICE' as const, id: 'd', supplierName: 'Bidfood', categoryCode: 'Cost of Sales — Food', totalPence: -8_840, taxPence: 0 })).toBeNull();
});
...
test('a document with no category refuses with NT-PUB-001 naming the field', () => {
  ...
  const refusal = checkPublishMinimum({ docType: 'INVOICE' as const, id: 'doc_004', supplierName: 'Currys', categoryCode: null, totalPence: 129_900, taxPence: 21_650 });
...
test('a sales document with no supplier refuses too — the minimum is all three fields', () => {
  ...
  const refusal = checkPublishMinimum({ docType: 'INVOICE' as const, id: 'doc_010', supplierName: null, categoryCode: 'Sales — Delivery', totalPence: 284_155, taxPence: 47_359 });
...
test('a whitespace-only supplier or category is an extraction artefact, not an answer', () => {
  const refusal = checkPublishMinimum({ docType: 'INVOICE' as const, id: 'd', supplierName: '   ', categoryCode: '', totalPence: 100, taxPence: 0 });

  expect(refusal?.missing).toEqual(['supplier', 'category']);
});

test('every missing field is named, in plain English', () => {
  const refusal = checkPublishMinimum({ docType: 'INVOICE' as const, id: 'd', supplierName: null, categoryCode: null, totalPence: null, taxPence: null });
...
test('one bad item refuses the whole batch, and EVERY bad item is reported', () => {
  const outcome = previewPublishBatch([
    ...READY_BATCH,
    { docType: 'INVOICE' as const, id: 'doc_004', supplierName: 'Currys', categoryCode: null, totalPence: 129_900, taxPence: 21_650 },
    { docType: 'INVOICE' as const, id: 'doc_010', supplierName: null, categoryCode: 'Sales — Delivery', totalPence: 284_155, taxPence: 47_359 },
  ]);
