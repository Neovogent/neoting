# archive-vault-search

**Lane L** · **Source of Truth:** SoT §4 Stage 11, §10 · **Owner:** see the project board

## Purpose

Archive semantics, move-between-entities with the addressee warning, full-text search, and the Document Vault.
...
- A move between client workspaces passes Review to Approve and is audit-logged against both workspaces. Originals in S3 are immutable; corrections are metadata.
...
```bash
pnpm --filter @neoting/api test -- archive-vault-search
```
