...
- **`buildExecutorRegistry(deps)`** — total over the contract's `ProposalKind`
  ...
  update-coding since METH S3 #122, publish.batch since S10), seven honest
  holes throwing `ProposalNotImplementedError` by name — the remaining #81 four
  (`move-business`, `reprocess`, `reject`, `split`) plus the METH Stage 2 kinds
  still unbuilt (#120): `chase.send` (METH S8), `bank.confirm-match` (S11),
  `rule.create` (S13), each typed off the generated payload models. **No
  controller imports the proposals directory** — a test walks every
  `*.controller.ts` and asserts it; the provider-side half is upheld in
  `approvals.module.ts` (registry built inside the service factory, no token).
  The `deps` argument arrived with S10 and is **required, not optional with a
  fallback**: a registry that quietly degraded a built executor back to a hole
  because a call site forgot an argument is the failure the mapped type exists
  to prevent, moved one level out.
...
- [ ] The seven unimplemented executors — the remaining #81 four
      ...
      issue) and the METH kinds still open (`chase.send` → S8,
      `bank.confirm-match` → S11, `rule.create` → S13). The registry already
      types and names them all. `update-coding` landed with the engine
      (METH S3, #122); `publish.batch` landed in METH S10.
