...
async function extract(filename: string) {
  ...
  return extractor.extract({ filename, byteHash: HASH, s3Key: null, mimeType: null, practiceId: 'prac_1' });
}
...
test('an unknown filename gets a deterministic hash-derived generic profile', async () => {
  const a = await extractor.extract({ filename: 'mystery.pdf', byteHash: HASH, s3Key: null, mimeType: null, practiceId: 'prac_1' });
  const b = await extractor.extract({ filename: 'mystery.pdf', byteHash: HASH, s3Key: null, mimeType: null, practiceId: 'prac_1' });
  if (!a.ok || !b.ok) throw new Error('expected ok');
...
test('a different hash yields a different generic supplier and total', async () => {
  const a = await extractor.extract({ filename: 'mystery.pdf', byteHash: 'a'.repeat(64), s3Key: null, mimeType: null, practiceId: 'prac_1' });
  const b = await extractor.extract({ filename: 'mystery.pdf', byteHash: 'b'.repeat(64), s3Key: null, mimeType: null, practiceId: 'prac_1' });
  if (!a.ok || !b.ok) throw new Error('expected ok');
