...
export interface BusinessPortalSession {
  ...
  /**
   * `expenseClaim` (review item 50) is last and optional so every existing
   * caller is unchanged: an ordinary upload does not mention it, and the
   * server treats an absent mark as "the company paid".
   */
  upload(
    file: File,
  ...
  const send = useCallback(
    async (
      ...
      try {
        ...
        await sendPortalUpload(token, file, transactionId, note, expenseClaim);
        await refresh();
