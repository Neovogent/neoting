    ...
    <div className="w-full max-w-3xl border border-white/5 rounded-[32px] bg-card shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[12px] text-zinc-500 mt-1 font-semibold uppercase tracking-wider truncate">
              {doc.clientName} • {doc.date} • {currency(doc.total)}
            </p>
          </div>
        </div>
        <div className="flex flex-col items-end gap-2 shrink-0">
          <StatusPill doc={doc} />
          <span className="text-[11px] text-zinc-600 font-semibold uppercase tracking-wider">via {doc.source}</span>
        </div>
      </div>
      ...
      {isProcessing ? (
        ...
        <div className="p-6 text-sm text-zinc-400">
          <p className="font-semibold text-white mb-1">No fields extracted</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_1.25fr]">
          ...
          <div className="p-6">
            <div className="flex items-center justify-between mb-4">
              <span className="text-[11px] font-bold text-zinc-500 uppercase tracking-widest">Extracted fields</span>
              <span className="text-[11px] font-semibold text-zinc-600">click any value to correct</span>
            </div>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="mt-0.5 flex items-center gap-2">
                      ...
                      <span className="text-[11px] text-zinc-600 font-semibold">{Math.round(f.confidence * 100)}% confident</span>
                    </div>
                  </div>
                  ...
                  {doc.lineItems.map((li, i) => (
                    <div key={i} className="px-4 py-3 flex items-center justify-between gap-3 text-[13px]">
                      ...
                      <span className="text-white font-bold shrink-0">
                        {li.quantity} × {currency(li.total / li.quantity)}
                      </span>
                    </div>
                  ))}
                </div>
...
function StatusPill({ doc }: { doc: Document }) {
  if (doc.status === 'ready') return <Pill tone="green">Ready</Pill>;
  if (doc.status === 'review') return <Pill tone="amber">To Review — {doc.statusNote}</Pill>;
  if (doc.status === 'rejected') return <Pill tone="red">Rejected</Pill>;
  if (doc.status === 'published') return <Pill tone="blue">Published</Pill>;
  return <Pill tone="neutral">Processing</Pill>;
}
