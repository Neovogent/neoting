...
export function detectDuplicates(
  ...
  for (let i = 0; i < candidates.length; i++) {
    for (let j = i + 1; j < candidates.length; j++) {
      ...
      const verdict = compare(a, b, settings);
...
function compare(a: Document, b: Document, settings: DedupeSettings): Verdict {
  ...
  if (supplierMatch > 0.9) {
    ...
    signals.push('Same supplier');
  } else if (supplierMatch > 0.6) {
    score += WEIGHTS.supplier * 0.5;
    signals.push('Supplier names close');
  }
  ...
  if (gap === 0) {
    ...
    signals.push(`Dates ${gap} day${gap === 1 ? '' : 's'} apart`);
  }
  ...
  if (textMatch > 0.5) {
    ...
    signals.push(`OCR text ${Math.round(textMatch * 100)}% similar`);
  }
  ...
  if (refA && refB && refA === refB) {
    ...
    signals.push('Same document reference');
  }

  /* ── file hash: the same file sent twice ────────────────────────────────── */
  if (fileHash(a) === fileHash(b)) {
    score += WEIGHTS.fileHash;
    signals.push('File hash match');
  } else {
    signals.push('File hash differs');
  }
  ...
  if (isImage(a) && isImage(b) && perceptualHash(a) === perceptualHash(b)) {
    ...
    signals.push('Perceptual hash near-match');
  }

  /* ── the two gaps this exists to close ──────────────────────────────────── */
  if (a.uploader !== b.uploader) signals.push('Different uploaders');
  if (typeOf(a) !== typeOf(b)) signals.push(`Cross-type: ${typeOf(a)} ↔ ${typeOf(b)}`);
 ...
 * is the better tell.
 */
function typeOf(d: Document): string {
  if (d.total < 0) return 'Credit note';
...
const side = (d: Document): DuplicatePair['left'] => ({
  id: d.id,
  label: `${d.supplier} ${typeOf(d).toLowerCase()}`,
  type: typeOf(d),
  total: d.total,
  date: d.date,
  uploader: d.uploader,
