...
describe('detectDuplicates — the seeded pairs', () => {
  const pairs = detectDuplicates(everything);
...
describe('detectDuplicates — what it refuses to pair', () => {
  it('never pairs the same invoice sent to two different companies', () => {
    const pairs = detectDuplicates([doc({ id: 'a' }), doc({ id: 'b', clientId: '2', clientName: 'Cosmo' })]);

    expect(pairs).toEqual([]);
  });

  it('never pairs money in with money out', () => {
    const pairs = detectDuplicates([doc({ id: 'a', kind: 'cost' }), doc({ id: 'b', kind: 'sales' })]);

    expect(pairs).toEqual([]);
  });

  it('leaves a document still being read alone — there is nothing to compare on yet', () => {
    const pairs = detectDuplicates([doc({ id: 'a' }), doc({ id: 'b', status: 'processing' })]);

    expect(pairs).toEqual([]);
  });

  it('will not pair two different spends that merely share a supplier and a date', () => {
    const pairs = detectDuplicates([doc({ id: 'a', total: 1420.5 }), doc({ id: 'b', total: 380 })]);
...
describe('detectDuplicates — the settings that govern it', () => {
  it('honours a tighter threshold, and reports the pair it does keep', () => {
    const strict = detectDuplicates(everything, { ...DEFAULT_DEDUPE_SETTINGS, threshold: 0.8 });

    expect(strict.map((p) => p.id)).toEqual(['dup-d1-d1b']);
  });

  it('scores a copy lower once the date tolerance no longer covers the gap', () => {
    const loose = detectDuplicates(everything, { ...DEFAULT_DEDUPE_SETTINGS, dateToleranceDays: 5 });
    const tight = detectDuplicates(everything, { ...DEFAULT_DEDUPE_SETTINGS, dateToleranceDays: 1 });
  ...
  it('tolerates a same-day copy whatever the tolerance is set to', () => {
    const pairs = detectDuplicates(everything, { ...DEFAULT_DEDUPE_SETTINGS, dateToleranceDays: 0 });
