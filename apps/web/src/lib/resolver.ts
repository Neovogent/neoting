 ...
 */
const PATTERNS: { intent: Intent; test: RegExp; response: string }[] = [
  {
    intent: 'ADD_CLIENT',
    test: /\b(add|create|onboard|set ?up)\b.*\b(client|company|business)\b|\badd\b.*\bas a client\b/i,
    response: "I'll set that client up. Complete the intake form below and I'll take it through review before anything is created.",
  },
  {
    intent: 'CHASE_MISSING',
    test: /\b(chase|remind|nudge|follow up)\b/i,
    response: "I've drafted the chase. Nothing sends until you read the review and approve it.",
  },
  {
    intent: 'SHOW_MISSING',
    test: /\b(missing|outstanding|unverified|no receipt|without paperwork)\b/i,
    response: "I've scanned bank feeds, supplier statements and recurring patterns. Here's what's missing:",
  },
  {
    intent: 'SHOW_DUPLICATES',
    test: /\b(duplicate|dupe|same invoice|twice)\b/i,
    response: 'Here are the flagged duplicate pairs with their similarity signals.',
  },
  {
    intent: 'SHOW_MATCHES',
    test: /\b(match|matched|reconcil|bank match)\b/i,
    response: 'Here are the current document ↔ transaction links, with match confidence on each.',
  },
  {
    intent: 'SHOW_REJECTED',
    test: /\b(rejected|failed|failure|error|bounce)\b/i,
    response: 'Every failure lands here with a reason and a retry — nothing vanishes silently.',
  },
  {
    intent: 'SHOW_APPROVALS',
    test: /\b(approval|approvals|awaiting approval|pending approval|need approving|approve queue)\b/i,
    response: 'Here is the approval queue, oldest first.',
  },
  {
    intent: 'APPROVE_ITEMS',
    test: /\b(approve)\b.*\b(items|claims|costs|invoices|bills|all|pending|under)\b/i,
    response: "I've prepared the approval batch. Read the review to see exactly what will be approved.",
  },
  {
    intent: 'CREATE_RULE',
    test: /\b(rule|whenever|always code|code (it|them)|auto-?publish)\b/i,
    response: "I've parsed that into a rule. It activates only after you review and approve it.",
  },
  {
    intent: 'PUBLISH',
    test: /\b(publish|push|send to (xero|quickbooks|qbo|sage|freeagent))\b/i,
    response: "Here's the publish batch with gross and VAT totals. Read the review before approving.",
  },
  {
    intent: 'INVITE_USER',
    test: /\b(invite|add)\b.*\b(user|colleague|team member|staff|approver)\b/i,
    response: 'Fill in the invite below — it goes through review before the invitation is sent.',
  },
  {
    intent: 'SHOW_AUDIT',
    test: /\b(audit|audit log|who approved|history of changes|trail)\b/i,
    response: 'Every approval is recorded with who, when, and what was shown at the time.',
  },
  {
    intent: 'SHOW_ANALYTICS',
    test: /\b(analytics|stats|metrics|how many|report|throughput|performance)\b/i,
    response: 'Here are the document-pipeline metrics. (Ledger reporting is out of scope for this product.)',
  },
  {
    intent: 'REVIEW_DOCUMENT',
    test: /\b(review|open|show me|check|look at)\b.*\b(document|receipt|invoice|bill)\b|\bextraction\b/i,
    response: 'Here it is. Every field shows its confidence and provenance — click any value to correct it.',
  },
  {
    intent: 'SHOW_INBOX',
    test: /\b(inbox|to review|ready|processing|costs|sales|documents)\b/i,
    response: 'Here is the inbox. Sort or bulk-select any of these.',
  },
];

export function classifyLocally(text: string): { intent: Intent; response: string } {
