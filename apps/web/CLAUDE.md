...
⚠ **The card is BOUNDED and scrolls itself (2 Sep 2026), and both halves are the frame's job rather than a caller's.** It had neither: a dialog taller than the window — a document detail on a short viewport, which is most of them — ran off the bottom edge, and on the phone branch the sheet is anchored with `items-end` + `mt-auto`, where an overflowing item aligns by rules nobody should have to reason about and the scrim's own scrollbar is hidden by its two `[scrollbar-width]` utilities. Reported from the live app as a Path-to-Ready panel whose last action button was cut off with nothing to scroll. So the wrapper carries `max-h-full` and the children sit in an `overflow-y-auto overscroll-contain` box; the close button is deliberately a SIBLING of that box, so it neither scrolls away nor gets clipped. The trade is that the box clips what paints outside a card's own edges — a `shadow-2xl` at the left and right — and unreachable content is the worse of the two. The same wrapper now forces `[&>*]:w-full`, because a child that forgets `w-full` shrink-wraps to its own content and reads as a stray pill on the scrim; `RequestStatementDialog` was the one call site doing it, and it was additionally drawing no `bg-card` at all.

⚠ **The 2 Sep fix shipped broken TWICE, with every one of its classes present and asserted (items 23+40, fixed 5 Sep 2026), and the root cause is one flexbox rule.** Nearly every dialog card carries `overflow-hidden` for its rounded corners, and flexbox zeroes the *automatic minimum size* of a flex item whose overflow is not `visible` — so inside the frame's column-flex scroll box the card did not overflow and get scrolled, it was **shrunk to fit** and its own `overflow-hidden` clipped the tail. `scrollHeight === clientHeight`: nothing to scroll, last button unreachable, publishing impossible (the publish dialog with the server's review mounted was the release-blocking instance; the intake Review step was the other reported one). The fix is `[&>*]:shrink-0` on the frame's scroll box, sibling to `[&>*]:w-full`. The same sweep fixed the own-chrome dialogs that could not scroll at all — `ConfirmStep`, `OffboardClientDialog` and InboxesView's two synthetic confirm dialogs now centre/anchor the card by **auto margins on a scrolling scrim** instead of `items-center`/`items-end` (auto margins collapse to zero on overflow, so a too-short viewport scrolls instead of clipping both ends), and `BusinessPortalLauncher`'s InviteForm branch got the scroll box its bounded card assumed. The own-chrome dialogs that already used `items-start` + a scrolling scrim (AnalysisModal, ChaseModal, DuplicateModal, StatementModal, the view-local previews) were sound all along — a **row**-flex scrim only overflows on the cross axis, where nothing shrinks.

**The class contract in `CodingProposalModal.test.tsx` is now only a tripwire for someone deleting a class — it can never prove the mechanism, because jsdom computes no layout and this bug class lives entirely in layout.** The guard that actually runs layout is `scripts/measure/modal-reachability.mjs`: a no-dependency CDP smoke that drives a real headless Chromium at 1280×500 through the two dialogs that were reported broken (intake → Review step, publish → staged server review) and fails unless the last button is genuinely scrollable into view and hit-testable — it also refuses to pass if the dialog happens to fit, so it cannot rot into a non-proof. Run it against the local seeded stack: `CHROME_EXE=<chromium> node scripts/measure/modal-reachability.mjs`. Any future change to the Modal frame's classes must come with a run of that script; a green jsdom suite says nothing here.
