...
6. Accessibility on every PR: full keyboard path, visible focus, `aria-live="polite"` on chat updates, contrast from tokens, error text never colour-only. Run axe locally before requesting review. `jsx-a11y` (recommended set) and `react-hooks` (`rules-of-hooks` + `exhaustive-deps`) are now in `eslint.config.js`, all at error — **but the sweep of the pre-existing findings is still owed**: turning the rules on surfaced 39 (28 a11y — mostly clickable `<div>`s missing keyboard handlers — and 11 `exhaustive-deps`, nine of them in `AppContext.tsx`), so `pnpm lint` is red until a human-planned pass fixes them properly. Do not shrink that number with disable comments; do not add to it.
